    ...
    return (
        <Container maxWidth="100%">
            <Paper elevation={0} sx={{ p: 4, mt: 4 }}>
                <Typography variant="h4" gutterBottom>{data.title} <span style={{color:'gray',fontSize:'1.2rem',fontStyle:'italic'}}>{data.city}</span></Typography>
                <Box className='box' sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                    {data && data.image && data.image[currentImageIndex] && (
                        <>
                            <img src={data.image[currentImageIndex]} alt={data.alt} style={{ width: '100%', height: '500px',objectFit:'cover', borderRadius:'20px' }}/>
                            <CarouselImage className='carousel' steps={data.image.length} activeStep={currentImageIndex} handleNext={goToNextImage} handleBack={goToPreviousImage} />
                        </>
                    )}
